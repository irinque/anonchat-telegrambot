...
from settings.messages import message_start_ban, message_start_exists, message_start_firstvisit
...
async def command_start(message: Message, bot: Bot):
    if db.user_exists(message.from_user.id):
        if db.user_check_ban(message.from_user.id):
            await bot.send_message(message.from_user.id, message_start_ban, parse_mode="Markdown")
        else:
